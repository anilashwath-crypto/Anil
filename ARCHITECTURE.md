...
- Tab sections (shown/hidden by the bottom nav):
  ...
  - `#tab-water` — pump & borewell, 6 irrigation zones, soil field stations, fertigation + auto-dosing diffuser
...
- Component classes: `.card .kpi .seg .chart-wrap .tooltip .zone .switch .stat .pill
  .chip .badge .param .task .cam .alert .herd .forecast .insights`.
...
- `state` — single source of truth: `lang`, `metric`, `series{moisture,temp,humidity,wind}`
  (49 points = 24 h at 30 min), `soil[2]` (pH/EC/soil-temp stations), `diffuser`, `tank`,
...
| `renderAlerts()` | derives the alert list from state each pass: standing heat, zone below trigger, tank low, pH out of range, EC-guard pause, termite activity, weed pressure, high wind, shed heat stress; per-alert dismiss |
...
### Events
- One delegated `document` click handler: bottom-nav tabs → chart metric segments →
